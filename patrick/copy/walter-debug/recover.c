...
double dmoved(planet a[], vector b[])
{
  double dist=0;
  for (int i=0; i<N; i++)
  {
    dist += sqrt((a[i].vel-b[i])*(a[i].vel-b[i]));
  }
  return dist;
}
...
int main(int nParam, char **paramList){
	...
	long int anim_t;
			...
			vector sep_initial; double moved=0;
	for(i=0; i<LVL; i++)
	{
	...
	for(double power = 1; power<10; power+=0.1)
	{
		int factor=100;
		double bigstep=pow(10,-power);
		double smallstep=bigstep/factor;
		fError=0;
			for(i=0; i<N; i++)//Store initial positions so we can reset after 100 small Omelyan steps
			{
			...
			printf("Initial distance moved (should be 0): %e\n",dmoved(BD,velInitial));
		//	printf("t: %e\n", t);
			//printf("Initial positions stored, now taking 100 timesteps\n");
			for(i=0; i<factor; i++)//100 timesteps with small dt, call this the exact answer
			{
				dt=smallstep;
...
//				for(j=0; j<N; j++)
//				{
//					printf("i: %d -- BD[%d]: %e %e %e\n", i, j, BD[i].pos.x, BD[i].pos.y, BD[i].pos.z);
//				}
			}
			printf("Distance moved after exact         : %e\n",dmoved(BD,velInitial));
			for(i=0; i<N; i++)//Store exact positions after 100 small Omelyan steps
			{
				posExact[i]=BD[i].pos;
				velExact[i]=BD[i].vel;
			}

			for(i=0; i<N; i++)//Reset positions to before 100 steps
			{
				BD[i].pos=posInitial[i];
				BD[i].vel=velInitial[i];
			}
			...
			printf("Distance moved after large step    : %e\n",moved=dmoved(BD,posInitial));
		double fError = dmoved(BD,velExact);                         
		printf("fractional error %e, total runtime %e, distance moved %e\n", fError/bigstep, bigstep,moved);
                printf("AXIS %e %e\n",bigstep,fError/bigstep);
	}
}
