...
int main(int nParam, char **paramList){
	...
	for(dt=.1; dt/= 10; dt>1e-8)
	{
		printf("dt: %e\n", dt);
		fError=0;
		for( t=0; t<1; t+=100*dt )
		{
		//	printf("t: %e\n", t);
			...
			for(i=0; i<100; i++)//100 timesteps with small dt, call this the exact answer
			{
				//printf("timesteps taken: %d\n", i);
...
//				}
			...
			//printf("100 timesteps completed, storing these positions in posExact\n");
		...
		fError/=(2/dt);//Divide fError by the number of timesteps taken to get average error over run time
		fprintf(fE_dt,  "----%e %e----\n", dt, fError);
		printf("%e %e\n", dt, fError);
	}
