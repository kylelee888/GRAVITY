...
double omelyan( planet BD[] , double t){
	...
	for( i=0; i<N; i++ ){
		...
		//Now account for Saturn
		rad = radius(BD[i].pos, zero);
//		printf("rad = %f\n", rad);
		BD[i].acc = BD[i].acc - G*M_Sat*BD[i].pos/(rad*rad*rad);	

		//Not sure what the next line is for
		quickF=BD[i].m*vecLength(BD[i].acc);//Should always be positive
		//Do newtonian calcs here
		for(j=0; j<i; j++)
		{
			rad = radius(BD[i].pos, BD[j].pos);
			vecF = (BD[i].pos - BD[j].pos) * G * BD[i].m * BD[j].m / (rad*rad*rad);
			newtF = radius(vecF, vecF);
		}
		//Splitting the for loop like this skips the current planet without needing an if statement	
		for(j=i+1; j<N; j++)
		{
			rad = radius(BD[i].pos, BD[j].pos);
			vecF = (BD[i].pos - BD[j].pos) * G * BD[i].m * BD[j].m / (rad*rad*rad);
		...
		fErr+=fabs(newtF)-quickF;//newtF should always be greater than or equal in magnitude to forceMagic, could use this for extra testing
	...
	//	fErr=0;
		newtF=0;
		quickF=forceMagic( regions[0], BD[i], BD );
...
//		printf("rad = %f\n", rad);
		...
		fErr+=newtF-quickF;//newtF should always be greater than or equal in magnitude to forceMagic, could use this for extra testing
...
int main(int nParam, char **paramList){
	...
	if( nParam < 9 ) {printf("\n!Usage: ./<this> <N> <dt> <frameskip> <initial velocity> <box width> <initial conditions width> <alpha> <tmax> \n\n"); exit(0);}
