...
void populateRegions(planet BD[], region regions[])//FIXME should this be void since it isn't returning anything?
{
	...
	for( i=0; i<LVL; i++ )
	{
		...
		for( j=0; j<N; j++ )
		{
			...
			regions[index].mass += BD[j].m;
			regions[index].com += BD[j].m*BD[j].pos;//not divided by total M yet
			NMBR[index]++;//might be a simpler way
	...
	for( i=0; i<LVL; i++ )
	{
		...
		for(j=0; j<n; j++)
		{
			plnt_num += NMBR[getArrayIndex(i,j)];
		}
	...
	for( i=0; i<LVL; i++ ){
		...
		for(j=0; j<n; j++)
		{
			plnt_mass += regions[getArrayIndex(i,j)].m;
		}
		printf("The total mass inside boxes in level %d is %f with M = %f\n",i,plnt_mass,getTotalMass(BD);//FIXME Could probably just calculate total mass once at beginning of program
	}
	...
	for( i=0; i<LVL; i++ ){
		...
		plnt_COM.z = 0;
		for( j=0; j<n; j++ ){
			plnt_COM = plnt_COM + regions[arrayIndex(i,j)].com*regions[arrayIndex(i,j).m];//COM*MASS is the pos*mass
		}
		plnt_COM = plnt_COM / mass(BD); //can divide by mass(BD) here as it was proved the mass is good
		printf("The COM of boxes in level %d is (%f,%f,%f) with COM = (%f,%f,%f)\n",i,plnt_COM.x,plnt_COM.y,plnt_COM.z,com.x,com.y,com.z);
	}
}
