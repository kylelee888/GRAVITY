...
//	printf("!POPREG: finished initializing shit\n"); 
	for( i=0; i<LVL; i++ )//loop through all levels to find what box each planet is on on each level
	{
	  //printf("!POPREG: working on level %d\n",i);
		...
		for( j=0; j<N; j++ )
		{
			...
			if(regions[index].numPln == 0){//we are looping through all planets, if the box a planet is in
			  //says it doesnt have a planet in it, then it is the first time we have seen this box. rest its
			  //values
