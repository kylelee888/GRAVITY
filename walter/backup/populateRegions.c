...
void populateRegions(planet BD[], int totalIndeces)
{
	...
	for( i=0; i<LVL; i++ )
	{
        if (DEBUG) printf("!POPREG: working on level %d\n",i);
		...
		for( j=0; j<N; j++ )
		{
			...
			b = BD[j].pos.y*n/L;
			c = BD[j].pos.z*n/L;
                        clamp (a, n); // EDIT: need to ensure that we don't drive off our list of panels
                        clamp (b, n);
                        clamp (c, n);
			...
			if (DEBUG) printf("!POPREG:   Planet %d (%.2f,%.2f,%.2f) is in region %d:%d,%d,%d/%d, which now has %d planets of mass %f\n",j,BD[j].pos.x,BD[j].pos.y,BD[j].pos.z,i,a,b,c,index,regions[index].numPln,regions[index].mass);
