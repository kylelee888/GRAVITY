...
{
  int lvl, count, pside, cside, iP, jP, kP, iC, jC, kC, parentBox, parentIndex, childBox, childIndex;//b for "boxes per side", _P for Parent, _C for Child, boxOfLvl for the box number at a given level, ind for index in the large array
  
  for(lvl=0; lvl<maxLvl; lvl++)
  {
    pside=pow(2,lvl);
    if (DEBUG) printf("!TREE: Start level %d\n",lvl);
	...
	for(kP=0; kP<pside; kP++)//k direction
	{
	  ...
	  if (DEBUG) printf("!TREE: Region %d:%d,%d,%d/%d: \n",lvl,iP,jP,kP,parentIndex);
          count=0;
	  for(iC=iP*2; iC<=iP*2+1; iC++)//Loop over child boxes in i direction
	  {
		...
		if (DEBUG) printf("!TREE:   Child %d is %d:%d,%d,%d/%d\n",count,lvl+1,iC,jC,kC,childIndex);
		count++;
	      }
	    }
	  }
